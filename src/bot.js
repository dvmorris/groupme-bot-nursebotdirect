...
bot.respond = function (request) {
  ...
  if (!(typeof botConfig === 'object' && botConfig) && groupID.indexOf('+') === -1) {
  ...
  if (typeof msg === 'string') {
    ...
    if (msg.substring(0, botName.length + 1) === botName + ' ') {
      ...
      this.processCommand(botConfig.groupLocalID, request.name, msg, request, function (response) {
        if (response !== null) {
          if (groupID.indexOf('+') > -1) {
            this.postDirectMessageToUserID(request, response);
          } else {
...
bot.processCommand = function (groupLocalID, userDisplayName, message, originalRequest, cb) {
  ...
  command[1](groupLocalID, userDisplayName, tokens, originalRequest, cb);
};
...
bot.postDirectMessageToUserID = function (request, text) {
  var options, body, botReq;

  options = {
    hostname: 'api.groupme.com',
    path: '/v3/direct_messages?token=' + config.access_token,
    method: 'POST',
    headers: {
  ...
  console.log('sending direct message to ' + request.sender_id);
  ...
  botReq.on('error', function(err) {
    console.log('error posting direct message '  + JSON.stringify(err));
  });
  botReq.on('timeout', function(err) {
    console.log('timeout posting direct message '  + JSON.stringify(err));
  });
  botReq.end(JSON.stringify(body));
};
